                                 ...
                                 double factor_lj, double &fforce)
{
  ...
  // this function never works correctly
  // single() takes the charges defined by user in pair_write; 
  // for DC atoms user should define -DP charge (that is damped) in in.lmp, 
  // but for coul interaction it should be DC charge (that is in data.lmp)
  ...
  if (rsq < cut_coulsq) {
    ...
    if (!ncoultablebits || rsq <= tabinnersq) {
      ...
      u = 1. - t;
      erfc = t * (1.+u*(B0+u*(B1+u*(B2+u*(B3+u*(B4+u*B5)))))) * expm2;
      // here should be q of DC (for core) and q of DP (for particle)
  ...
  //here should be q of -DP (for core) and q of DP (for particle)
  dqi=atom->q[i];
  dqj=atom->q[j];
